defmodule AdventOfCode.Day22 do
  ...
  defp part_two_sub([h1 | t1], [h2 | t2], p1_past, p2_past) do
    if [h1 | t1] in MapSet.new(p1_past) || [h2 | t2] in MapSet.new(p2_past) do
      ...
      case winner do
        1 -> part_two_sub(t1 ++ [h1, h2], t2, [[h1 | t1] | p1_past], [[h2 | t2] | p2_past])
        2 -> part_two_sub(t1, t2 ++ [h2, h1], [[h1 | t1] | p1_past], [[h2 | t2] | p2_past])
      end
    end
  end
  ...
  defp part_two_round([h1 | t1], [h2 | t2], p1_past, p2_past) do
    if [h1 | t1] in MapSet.new(p1_past) || [h2 | t2] in MapSet.new(p2_past) do
      score([h1 | t1])
    else
      winner = if h1 <= length(t1) && h2 <= length(t2),
      ...
      case winner do
        1 -> part_two_round(t1 ++ [h1, h2], t2, [[h1 | t1] | p1_past], [[h2 | t2] | p2_past])
        2 -> part_two_round(t1, t2 ++ [h2, h1], [[h1 | t1] | p1_past], [[h2 | t2] | p2_past])
      end
    end
  end
